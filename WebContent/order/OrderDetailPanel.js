Ext.namespace("order.detail");

order.detail.url = {
	 query : '/order/order!queryPagedDetail.do'
	,save : '/order/order!saveOrderDetail.do'
	,remove : '/order/order!removeDetail.do'
...
order.detail.MainPanel = function(config) {
	this.config = config?config:{};
};

order.detail.MainPanel.prototype = {
		...
		if (!this.detailGrid) {
			...
			var gridStore = new Ext.data.GroupingStore({
	            ...
	            proxy : new Ext.data.HttpProxy({
	                url: Ext.getDom("root").value + order.detail.url.query,
	                method: 'POST'
	            }),
	            groupField: 'orderId'
	        });
							...
							var productChooser = new ProductChooser({
								...
								callBack:function(records){
									Ext.each(records, function(rec,index){
										rec.set('orderId',_this.config.orderId);
