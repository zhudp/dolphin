Ext.namespace("people.card");

/**
 * 居住证打印窗口
 * @author jinnie 2010-11-22
 * @param config: {
 * 		title		打印予预览页的title
 * 		width		打印予预览页的宽度
 * 		height		打印予预览页的高度
 ...
 * }
 */
people.card.PrintCard = function(config) {
	this.config = config;
};
people.card.PrintCard.prototype = {
	...
	show: function() {
		...
		var printIframe = document.getElementById('_print_charter_area');
	...
	getPrintArea: function() {
		if(!this.printArea) {
			...
			var printArea = new Ext.Panel({
				border: false,
				html: '<div id="progressShow" align="center"></div><iframe id="_print_charter_area" scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>'
			});
