var PurchaseChooser = function(config) {
	this.config = config==null?{}:config;
	this.config.queryUrl = '/purchase/purchase!queryPaged.do';
				...
				listeners:{
					'specialkey' : {
						fn:function(field, e) {
							if (e.getKey() == e.ENTER) {
								this.dataGrid.load({
									purchaseName : filterField.getValue()
								});
							}
						},
						scope: this
		...
		if(!this.dataGrid) {
			...
			this.dataGrid = new Ext.grid.GridPanel({
				...
				cm: new Ext.grid.ColumnModel([ 
  					...
  					{header : "采购单名称",dataIndex : 'purchaseName',width : 60 },
  					...
  					{header : "供应商名称",dataIndex : 'supplierName',width : 60 },
  					{header : "下单日期",dataIndex : 'orderDate',width : 60 },
  					{header : "计划入库日期",dataIndex : 'planStoreinDate',width : 60 },
  					{header : "实际入库日期",dataIndex : 'realStoreinDate',width : 60 },
